...
int main(int argc, const char* argv[])
{
    ...
    if (argc == 3)
    {
        ...
        pNode = new asciiaccept;
        strcpy(argbuffer, psz[z++].c_str());
        argbuffer[sizeof(argbuffer)-1] = 0;
        alldigits(argbuffer, true);
        pNode->ascii = atoi(argbuffer);
        pHead = pNode;

        while (z < accs)
        {
            pNode->next = new asciiaccept;
            pNode = pNode->next;
            strcpy(argbuffer, psz[z++].c_str());
            argbuffer[sizeof(argbuffer)-1] = 0;
            alldigits(argbuffer, true);
    ...
    while (i < len)
    {
        ...
        while (j < to_read)
        {
            j += read(fd, (char*) (block + j), (to_read - j));
        }
